...
| Streamer search      | Add All included every subscription provider, but smaller direct services could not be selected as their own group. | The Movie Time `Other Streamers` tile selects direct subscription services outside the twelve individually displayed choices and combines with named services using OR. |
